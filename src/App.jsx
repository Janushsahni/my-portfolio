  ...
  return (
    <>
      ...
      >
        ...
        {showActionCenter && (
          <div 
            className="absolute bottom-16 right-4 w-80 bg-white/70 dark:bg-slate-800/80 backdrop-blur-3xl border border-white/40 dark:border-slate-600/40 shadow-2xl rounded-2xl p-5 z-[10000] animate-in slide-in-from-bottom-4 fade-in duration-200" 
            onClick={e => e.stopPropagation()}
          >
        ...
        >
          ...
          {startOpen && (
            <div 
              className="absolute bottom-2 left-1/2 transform -translate-x-1/2 w-[90vw] md:w-[600px] h-[500px] max-h-[80vh] bg-white/80 dark:bg-slate-800/80 backdrop-blur-2xl border border-white/40 dark:border-slate-600/40 shadow-2xl rounded-xl p-6 flex flex-col z-[1000] animate-in slide-in-from-bottom-4 duration-200"
              onClick={e => e.stopPropagation()}
            >
        ...
        {/* Taskbar */}
        <div className="h-14 w-full shrink-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-3xl border-t border-white/40 dark:border-slate-700/40 z-[999] grid grid-cols-3 px-2 md:px-4 shadow-[0_-5px_20px_rgba(0,0,0,0.1)] relative" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex items-center justify-center gap-1 md:gap-2 overflow-x-auto no-scrollbar">
            <button 
              ...
              className={`p-2 rounded-lg hover:bg-white/50 dark:hover:bg-slate-700 transition-colors ${startOpen ? 'bg-white/80 dark:bg-slate-600 shadow-inner' : ''}`}
            >
            ...
            {windows.map(win => (
              <button
                ...
                className={`h-10 w-10 rounded-lg transition-colors relative flex items-center justify-center flex-shrink-0 group ${activeWindow === win.id ? 'bg-white/80 dark:bg-slate-600 shadow-inner' : 'hover:bg-white/50 dark:hover:bg-slate-700'}`}
              >
                <win.icon size={22} className={win.taskbarColor} />
                <div className={`absolute bottom-0 h-1 rounded-t-sm transition-all ${activeWindow === win.id ? 'w-4 bg-blue-500' : 'w-1.5 bg-gray-400 group-hover:w-3'}`}></div>
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center justify-end gap-2 md:gap-3">
            <div 
              className={`flex items-center gap-2 md:gap-3 px-2 py-1.5 rounded cursor-pointer transition-colors text-gray-800 dark:text-gray-200 ${showActionCenter ? 'bg-white/80 dark:bg-slate-600 shadow-inner' : 'hover:bg-white/50 dark:hover:bg-slate-700'}`}
              onClick={(e) => { closeMenus(); setShowActionCenter(!showActionCenter); }}
            >
              <ChevronUp size={14} className="hidden sm:block"/>
              <Wifi size={14} />
              <Volume2 size={14} />
              <Battery size={14} />
            </div>
            <div className="flex flex-col items-end justify-center px-2 py-1 rounded hover:bg-white/50 dark:hover:bg-slate-700 cursor-default transition-colors text-[10px] md:text-xs text-gray-800 dark:text-gray-200 font-medium">
