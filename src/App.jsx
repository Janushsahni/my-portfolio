...
import { 
  ...
  ChevronUp, Code, Briefcase, ExternalLink, Download, Compass,import React, { useState, useEffect, useRef } from 'react';
import { 
  Folder, Mail, Github, Linkedin, 
  Globe, Terminal, Cpu, Wifi, Battery, BatteryFull,
  Volume2, Power, X, Minus, Square, Search,
...
const AboutApp = () => {
  return (
    <div className="p-6 pt-6 text-gray-800 dark:text-gray-200 h-full overflow-y-auto bg-slate-50/95 dark:bg-slate-900/95 pb-20 transition-colors duration-300">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8 bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm relative overflow-hidden transition-colors duration-300">
...
const ProjectsApp = () => (
  <div className="p-6 pt-6 bg-slate-50 dark:bg-slate-900 h-full overflow-y-auto pb-20 transition-colors duration-300">
...
const BlogsApp = () => (
  <div className="h-full flex flex-col bg-white dark:bg-slate-900 pt-4 md:pt-0 pb-10 md:pb-0 transition-colors duration-300">
  ...
  return (
    <div className="absolute inset-0 bg-black overflow-hidden font-sans select-none flex justify-center items-center">
      <div className="relative w-full h-full bg-black overflow-hidden shadow-2xl">
        ...
        {/* iOS Status Bar (Dynamically switches color if an app is open and it's light mode) */}
        <div className={`absolute top-0 w-full flex justify-between px-6 pt-3 text-xs font-semibold z-[90] pointer-events-none drop-shadow-md transition-colors duration-300 ${activeApp && !isDarkMode ? 'text-black drop-shadow-none' : 'text-white'}`}>
        ...
        <div 
          className={`absolute inset-0 flex flex-col transition-all duration-300 ${islandExpanded || showSearch || activeApp ? 'blur-sm scale-[0.98] opacity-80' : 'blur-none scale-100 opacity-100'}`}
        ...
        {/* --- ACTIVE APP FULL SCREEN (SUB-PAGE) --- */}
        <div className={`absolute inset-0 z-[80] transition-transform duration-[400ms] ease-in-out flex flex-col bg-white dark:bg-slate-900 overflow-hidden shadow-2xl ${activeApp ? 'translate-x-0' : 'translate-x-full pointer-events-none'}`}>
          ...
          {/* iOS Home Indicator */}
          <div className="absolute bottom-0 left-0 right-0 h-8 flex justify-center items-end pb-2 bg-gradient-to-t from-white dark:from-slate-900 via-white/80 dark:via-slate-900/80 to-transparent pointer-events-none z-50">
            <div 
              className="w-1/3 h-1.5 bg-black/90 dark:bg-white/90 rounded-full cursor-pointer pointer-events-auto active:scale-95 transition-all shadow-sm"
